...
const countMaterialsByMaxCategory = (materialList, maxList) => {
  ...
  materialList.forEach(item => {
    ...
    if (categoryId !== null && categoryId !== undefined) {
      ...
      if (maxCategoryId !== null && countMap.hasOwnProperty(maxCategoryId)) {
        ...
        console.log(`物料 ${item.partName || item.materialName} 的分类ID ${categoryIdStr} 无法匹配到Max分类`)
      }
    } else {
  ...
  console.log(`物料统计: 总数=${totalMaterials}, 有分类=${materialsWithCategory}, 无分类=${materialsWithoutCategory}`)
...
const loadData = async () => {
  ...
  try {
    const [maxList, materialList] = await Promise.all([
      fetchCategoryList(),
      fetchMaterialList()
    ])
    ...
    series: [
      {
        ...
        data: chartData.value.map((item, index) => ({
          ...item,
          itemStyle: {
            color: ['#409eff', '#67c23a', '#e6a23c', '#f56c6c', '#909399', '#9c27b0', '#00bcd4'][index % 7]
          }
        }))
      }
    ]
